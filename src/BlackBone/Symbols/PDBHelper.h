...
namespace blackbone
{
	// this class uses "diaguids.lib", which compiled with TLS support. Since we don't need
	// enable TLS and PDBHelper functionality, we just make it dummy.

	class PDBHelper
	{
	public:
		HRESULT Init(const std::wstring& file, ptr_t base = 0) { return -1; };
		HRESULT GetSymAddress(const std::wstring& symName, ptr_t& result) { return -1; };
		HRESULT CoCreateDiaDataSource() { return -1; };
		HRESULT PopulateSymbols() { return -1; };
	};
